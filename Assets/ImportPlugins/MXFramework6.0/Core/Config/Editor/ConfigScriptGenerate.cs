...
namespace Mx.Config
{
    public class ConfigScriptGenerate
    {
        ...
        private static void Initialize()
        {
            ...
            if (Directory.Exists(ConfigDefine.GetExternalConfigOutPath)) Directory.Delete(ConfigDefine.GetExternalConfigOutPath, true);
            Directory.CreateDirectory(ConfigDefine.GetExternalConfigOutPath);
        ...
        private static void CreateDatabaseScript(EnumCofigLoadType cofigLoadType, TextAsset textAsset)
        {
            ...
            string txtName = (ConfigDefine.Encrypt) ? StringEncrypt.GetStringMd5(textAsset.name) : textAsset.name;
            ...
            switch (cofigLoadType)
            {
                case EnumCofigLoadType.Streaming:
                    ...
                    configPath = ConfigDefine.GetExternalConfigOutPath + txtName;
                    break;

                case EnumCofigLoadType.Resources:

                    template = GetTemplate(ConfigDefine.TEMPLATE_DATABASE_RESOURECES_PATH);
                    configPath = ConfigDefine.CSV_RESOUCES_ENCRYPT_PATH + txtName + ".txt";
                    break;
            }
            ...
            template = template.Replace("$DataPath", "\"" + txtName + "\"");

            GenerateScript(textAsset.name + "Database", template);
            csvWrite(configPath, textAsset.text);
        }
        ...
        /// <summary>写入配置表文件</summary>
        private static void csvWrite(string textPath, string textAsset)
        {
            string data = (ConfigDefine.Encrypt) ? StringEncrypt.EncryptDES(textAsset) : textAsset;
